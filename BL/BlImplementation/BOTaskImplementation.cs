...
internal class BOTaskImplementation : IBOTask
{
    ...
    {
        if (doTask.EndOfActualWork is not null)
            return Status.Done;

        if (doTask.StartWorkDate is not null)
            return Status.Started;

        if (doTask.PlannedDateForStartingWork is not null)
    ...
    public void Add(BOTask bOTask)
    {
        ...
        try
        {
            newTaskId = _dal.Task.Create(doTask);
        }
        ...
        {
            DO.Dependency dOdependency = new DO.Dependency(0, newTaskId, BoDependency.NumOfTask, null);
    ...
    public BOTask? Read(int NumberOfTask)
    {
        ...
        {
            if (DoDependency is null || DoDependency.TaskNumberDepends != bOTask.NumOfTask)
                continue;

            DO.Task? dependenceOn = _dal.Task.Read(DoDependency.PreviousTaskDepends);
            if (dependenceOn is null)
                continue;
    ...
    public IEnumerable<BO.BOTask> ReadList(Func<BO.BOTask, bool>? filter = null)
    {
        ...
        foreach (var dotask in _dal.Task.ReadAll())
        {
            ...
            //Added a BO task to the collection
            taskList.Add(boTask);
        }
        ...
        //The return of the tasks who are true to the condition
        else
        {
    ...
    {
        DO.Task? doTask = _dal.Task.Read(NumberOfTask);

        if (doTask is null)
            throw new BO.BlDoesNotExistException($"Task with number = {NumberOfTask} does not exist");

        DO.Dependency? relatedDependency = _dal.Dependency.Read(dependency =>
            dependency.TaskNumberDepends == NumberOfTask ||
            dependency.PreviousTaskDepends == NumberOfTask);

        if (relatedDependency is not null)
            throw new BO.BlDeletionImpossibleException($"Task with number = {NumberOfTask} cannot be deleted while it is part of a dependency relationship");
