...
internal class BOEngineerImplementation : IBOEngineer
{
    ...
    public BO.BOEngineer? Read(int EngineerIdFromPl)
    {
        ...
        DO.Task? currentTask = _dal.Task.Read(task => task.EsponsibleEngineerId == EngineerIdFromPl);
        ...
        return new BO.BOEngineer() 
        {
            ...
            CurrentTaskId = currentTask?.NumOfTask,
            Nickname = currentTask?.Nickname
        };
    }
    ...
    public IEnumerable<BO.BOEngineer> ReadList(Func<BO.BOEngineer, bool>? filter = null)
    {
        ...
        foreach (var doengineer in doEngineerList)
        {
            ...
            BO.BOEngineer? boengineer = new BO.BOEngineer
            {
                ...
                SalaryPerHour = doengineer.SalaryPerHour,
                LeverOfEngineer = (BO.EngineerLevels?)doengineer.LeverOfEngineer,
                CurrentTaskId = doCurrentTask?.NumOfTask,
                Nickname = doCurrentTask?.Nickname
            };
    ...
    {
        BO.BOEngineer? bOEngineer = Read(EngineerId);

        if (bOEngineer == null)
            throw new BO.BlDoesNotExistException($"Engineer with number = {EngineerId} does not exist");

        if (bOEngineer.CurrentTaskId is not null)
            throw new BlDeletionImpossibleException($"Engineer with ID number = {EngineerId} cannot be deleted while assigned to task {bOEngineer.CurrentTaskId}");
        ...
        {
            throw new BlDoesNotExistException($"Engineer with number = {EngineerId} does not exist");
        }
